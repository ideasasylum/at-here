---
version: 1
slug: "src-render-html-ts"
primary_target: "src/render/html.ts"
related_targets: ["src/index.ts"]
---

Scope: the card page (`/{handle}` and `/{did}`), plus the home page (`/`) and error pages, all rendered by `src/render/html.ts`. Visitor mode: Operate.

Audience and job: someone arriving from a link the account owner shared, who wants to reach that person on one of the listed apps in one tap. Secondary: a curious person typing a handle on the home page.
...
Unresolved: an OpenGraph image per card; whether the Bluesky banner ever appears behind the horizon (default no).
